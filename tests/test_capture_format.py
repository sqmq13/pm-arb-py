...
def _write_single_record(frames_path: Path, idx_path: Path, payload: bytes, rx_mono_ns: int) -> None:
    with frames_path.open("ab") as frames_fh, idx_path.open("ab") as idx_fh:
        append_record(frames_fh, idx_fh, payload, rx_mono_ns)


def test_frames_round_trip(tmp_path: Path) -> None:
    frames_path = tmp_path / "shard_00.frames"
    idx_path = tmp_path / "shard_00.idx"
    payloads = [b"alpha", b"bravo"]
    with frames_path.open("ab") as frames_fh, idx_path.open("ab") as idx_fh:
        append_record(frames_fh, idx_fh, payloads[0], 123)
        append_record(frames_fh, idx_fh, payloads[1], 456)
...
def test_frames_truncation_detected(tmp_path: Path) -> None:
    ...
    _write_single_record(frames_path, idx_path, b"alpha", 123)
